...
{
    // ── Helper: filter transaksi berdasarkan "YYYY-MM" ──────────────
    private function byYearMonth(Builder $query, string $yearMonth): Builder
    {
        [$y, $m] = array_map('intval', explode('-', $yearMonth));
        return $query->whereYear('date', $y)->whereMonth('date', $m);
    }
    ...
    public function advanced()
    {
        ...
        // Pengeluaran per hari dalam minggu (avg) — pakai DAYOFWEEK & DAYNAME (MySQL)
        // Untuk SQLite gunakan strftime, tapi kita buat DB-agnostic dengan kolom virtual
        $driver = DB::connection()->getDriverName();

        if ($driver === 'sqlite') {
            $byDayOfWeek = Transaction::select(
                    DB::raw("CAST(strftime('%w', date) AS INTEGER) + 1 as day_num"),
            ...
            $byWeekOfMonth = Transaction::select(
                    DB::raw("CAST(strftime('%W', date) AS INTEGER) - CAST(strftime('%W', date('start of month', date)) AS INTEGER) + 1 as week_num"),
                    DB::raw('SUM(amount) as total')
                )
                ->where('user_id', $userId)
                ->where('type', 'expense')
                ->whereYear('date', (int) now()->format('Y'))
    ...
    public function forecast()
    {
        ...
        $currentIncome  = (float) Transaction::where('user_id', $userId)->where('type', 'income')
            ->whereYear('date', $curYear)->whereMonth('date', $curMonth)->sum('amount');
    ...
    public function insights()
    {
        ...
        $lastExpense = (float) Transaction::where('user_id', $userId)->where('type', 'expense')
            ->whereYear('date', $lastYear)->whereMonth('date', $lastMonth)->sum('amount');
        $lastIncome  = (float) Transaction::where('user_id', $userId)->where('type', 'income')
            ->whereYear('date', $lastYear)->whereMonth('date', $lastMonth)->sum('amount');

        // 1. Pengeluaran naik signifikan
        ...
        if ($topCategory && $curExpense > 0) {
            $catShare = $topCategory->total > 0
                ? (Transaction::where('user_id', $userId)->where('type', 'expense')
                    ...
                    ->whereYear('date', $curYear)->whereMonth('date', $curMonth)
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            $monthlyAvgExpense += (float) Transaction::where('user_id', $userId)->where('type', 'expense')
                ->whereYear('date', (int) $d->format('Y'))
                ->whereMonth('date', (int) $d->format('m'))
                ->sum('amount');
        }
        $monthlyAvgExpense /= 3;
    ...
    private function buildNetWorthHistory(int $userId, float $currentBalance, array $months): array
    {
        ...
        foreach ($reversed as $m) {
            $history[] = ['month' => $m['month'], 'net_worth' => round($balance, 0)];
            $balance  -= $m['savings'];
        }

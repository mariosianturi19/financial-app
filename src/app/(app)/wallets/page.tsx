...
import { Plus, Edit2, Trash2, TrendingUp, PiggyBank } from 'lucide-react';
...
const ICONS  = ['💳','🏦','💵','💎','🏠','🚗','📱','✈️','🎯','💡'];
const emptyForm = { name: '', balance: '', currency: 'IDR', color: '#0ea5e9', icon: '💳' };
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        boxShadow: hovered
          ? `0 20px 50px rgba(0,0,0,0.5), 0 0 0 1px ${wallet.color}22`
    ...
    >
      ...
      <div style={{ position: 'relative' }}>
        ...
        <NumberFlow
          ...
          style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 800, color: balance >= 0 ? 'var(--text-primary)' : 'var(--accent-rose)', letterSpacing: '-0.03em', lineHeight: 1 }}
        />
      </div>
    </motion.div>
  );
      ...
      <div>
        ...
        {form.balance && (
          <p style={{ fontSize: 11, color: 'var(--text-tertiary)', marginTop: 5 }}>
            = Rp {parseInt(form.balance || '0', 10).toLocaleString('id-ID')}
          </p>
        )}
      </div>
      <div>
...
export default function WalletsPage() {
  ...
  return (
    <div className="page-root">
      ...
      {/* Total balance hero */}
      {wallets.length > 0 && (
        <div className="card noise animate-fadeup" style={{ padding: '28px 32px', background: 'linear-gradient(135deg, #060d1b 0%, #0d1628 100%)', borderColor: 'var(--border-accent)', overflow: 'hidden', position: 'relative' }}>
